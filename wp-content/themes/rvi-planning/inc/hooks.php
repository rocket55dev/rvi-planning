 ...
 * @return array
 */
function r55_body_classes( $classes ) {
...
add_filter( 'body_class', 'r55_body_classes' );

/**
 * Flush out the transients used in r55_categorized_blog.
 *
 * @author WDS
 * @return string
 */
function r55_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'r55_categories' );
}
add_action( 'delete_category', 'r55_category_transient_flusher' );
add_action( 'save_post', 'r55_category_transient_flusher' );
 ...
 * @return string
 */
function r55_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'rocket55' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'r55_content_more_link' );
 ...
 * @return string
 */
function r55_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'rocket55' ) );
}
add_filter( 'excerpt_more', 'r55_excerpt_more' );
 ...
 * @return array
 */
function r55_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'r55_custom_mime_types' );
...
add_shortcode( 'svg', 'r55_display_svg' );
 ...
 * @return string
 */
function r55_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'r55_header_scripts' );
	...
	echo r55_get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', 'r55_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function r55_display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( 'r55_footer_scripts' );
...
add_action( 'wp_footer', 'r55_display_customizer_footer_scripts', 999 );
...
function r55_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'rocket55' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'rocket55' ) : esc_html__( 'tagged with', 'rocket55' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'rocket55' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'rocket55' ), $search_term );
	}
...
add_action( 'wp_head', 'r55_add_og_tags' );
 ...
 * @author Corey Collins
 */
function r55_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'r55_remove_archive_title_prefix' );

/**
 * Populate state_filter select field with unique state values from locations taxonomy.
 *
 * @param array $field The ACF field array.
 * @return array Modified field with choices.
 */
function r55_load_state_filter_choices( $field ) {
	$field['choices'] = array();
...
add_filter( 'acf/load_field/name=state_filter', 'r55_load_state_filter_choices' );

/**
 * Add anchor IDs to headings in post content for navigation.
 *
 * @param string $content The post content.
 * @return string Modified content with heading IDs.
 */
function r55_add_heading_anchors( $content ) {
...
add_filter( 'the_content', 'r55_add_heading_anchors' );
